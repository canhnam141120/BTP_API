    ...
    public class BookRepository : IBookRepository
    {
        ...
        public async Task<ApiResponse> searchBookByTitleAsync(string search, int page = 1)
        {
            ...
            if (search != null)
            {
                ...
                foreach (var s in listStr)
                {
                    var bookResult = await _context.Books.Include(b => b.User).Include(b => b.Category).Where(b => b.Title.ToLower().RemoveAccents().Contains(s) && b.IsReady == true && b.Status == StatusRequest.Approved.ToString()).ToListAsync();
                    books.AddRange(bookResult);
